...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    Description || jobDescription.trim().length < 50) {
    ...
    const { data: resume, error: resumeError } = await supabase.from('resumes').select('content_text, file_name, id').eq('user_id', user.id).order('created_at', { ascending: false }).limit(1).single()
    if (resumeError) {
    ...
    if (!resume) {
      ...
      return NextResponse.json({ error: 'no_resume', message: upload your master resume first before tailoring.' }, { status: 400 })
    }
    ...
    try {
      ...
      c.log(`✓ Prompt built: ${prompt.length} chars`)
      ...
      try {
        ...
        console.log(`✓ JSON psuccessfully. Keys: ${Object.keys(tailoredData).join(', ')}`)
      } catch (parseErr: any) {
    ...
    const { data: savedOutput, error: saveError } = await supabase.from('tailored_outputs').insert({ user_id: user.id, company_namempanyName || '', job_title: jobTitle || '', job_url: jobUrl || null, job_description: jobDescription, ats_score: tailoredData.analysis?.ats_score ?? null, key_matches: tailoredData.key_matches ?? [], key_gaps: tailoredData.key_gaps ?? [], tailored_sections: tailoredData.tailored_sections ?? [], top_changes: tailoredData.top_changes ?? [], cover_letter: tailoredData.cover_letter ?? '', analysis: tailoredData.analysis ?? {} }).select('id').single()
    if (saveError) {
    ...
    if (companyName && jobTitle) {
      ...
      const { error: appError } = await supabase.from('applications').insert({ user_id: user.id, company_name: companyName, job_title: jobTitle, job_url: jobUrl || null, job_description: jobDescription, status: 'draft', tailored_output_id: savedOutput?.id ?? null })
      if (appError) {
      ole.error('⚠️ Application record insert error (non-fatal):', appError)
      } else {
    ...
    return NextResponse.json({ success: true, outputId: savedOutput?.id, data: tailoredData, remaining: accessResult.remainingRequests - 1, tier: accessResult.userTier })
  } catch (error: any) {
    const totalDuration = Date.now() - startTime
    ...
    return NextResponse.json({ error: error.message ?? 'S went wrong. Please try again.' }, { status: 500 })
  }
}
